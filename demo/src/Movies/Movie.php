...
final class Movie
{
    ...
    public static function fromFile(string $path): self
    {
        ...
        $title = $slug;
        $year = null;
        if (preg_match('/^#\s+(.+?)(?:\s*\((\d{4})\))?\s*$/m', $content, $matches)) {
            $title = trim($matches[1]);
            $year = isset($matches[2]) ? (int) $matches[2] : null;
        }

        $director = null;
        if (preg_match('/^\*\*Director:\*\*\s*(.+?)\s*$/m', $content, $matches)) {
            $director = trim($matches[1]);
        }

        $imdb = null;
        if (preg_match('/^\*\*IMDB\*\*:\s*(\S+)/m', $content, $matches)) {
            $imdb = trim($matches[1]);
        }

        $cast = [];
        if (preg_match('/^##\s+Cast\s*$(.*?)(?=^##\s+|\z)/ms', $content, $matches)) {
        ...
        $plot = [];
        if (preg_match('/^##\s+Plot\s*$(.*?)(?=^##\s+|\z)/ms', $content, $matches)) {
        ...
        return new self($slug, $title, $year, $director, $imdb, $cast, $plot, $content);
    }
